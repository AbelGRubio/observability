"""Unit tests for API application settings defaults and CORS parsing behavior."""
...
class TestAppSettings(unittest.TestCase):
    """Test suite for observe_api.config.app_settings.AppSettings."""
    ...
    def test_default_values(self) -> None:
        ...
        """
        settings = AppSettings()
        self.assertEqual(settings.api_ip, "localhost")
        self.assertEqual(settings.api_port, 8000)
        self.assertEqual(settings.minutes_refresh_conf, 1)
        self.assertEqual(settings.cors_origins_, "*")
