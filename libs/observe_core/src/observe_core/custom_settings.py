...
class CustomSettings(BaseSettings):
    ...
    @classmethod
    def __str__(cls) -> str:
        """Return the lowercase settings name without the trailing 'settings'."""
    ...
    def conf_file(self, value: str | Path | None) -> None:
        """Set the configuration file path and update file-loading behavior."""
        if value is None:
    ...
    def reload_cfg(self) -> None:
        """Reload the CFG file and update matching model fields."""
        if not self.__use_conf_file__ or not self.__conf_file__:
            return

        # Load flattened values from the CFG file.
        cfg_data = load_cfg_file(self.__conf_file__)

        # Update only fields that exist in the settings model.
