...
The `infra/` folder centralizes infrastructure orchestration for the whole monorepo.

- From this folder, you can bring up the complete platform using a single `docker-compose`.
- It contains the shared observability and infrastructure definitions used by all applications.
- It is also the place where operational documentation and infrastructure-related docs are grouped.

In practice, `infra/` is the entry point for running a fully integrated local environment.

---

## Apps

The `apps/` folder contains the main repositories/services of the platform:

- `api`: backend APIs and business endpoints.
- `mcp`: MCP runtime and tool exposure layer.
- `agent`: agent application and orchestration flows.
- `front`: frontend application.

### Agent + LiteLLM

For `apps/agent`, we run `LiteLLM` as the LLM gateway/proxy.

- It helps control and monitor LLM usage costs generated by the app.
- It improves observability with centralized request/response telemetry and model usage insights.
- It gives better operational control (routing, limits, and auditing) across providers.
